...
//  }
  ...
  public Completable save(Sample sample) {
    if (sample.getId() == 0) {
      return Completable.fromSingle(sampleDao.insert(sample))
          .subscribeOn(Schedulers.io());
    } else {
      return Completable.fromSingle(sampleDao.update(sample))
          .subscribeOn(Schedulers.io());
    }

  }

  public Completable delete(Sample sample) {
    if (sample.getId() == 0) {
      ...
      return Completable.fromSingle(sampleDao.delete(sample))
          .subscribeOn(Schedulers.io());
